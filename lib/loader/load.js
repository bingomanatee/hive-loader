...
module.exports = function (cb, root, handlers) {
	...
	if (!root) {
		throw new Error('no root path defined for loader %s', util.inspect(this));
	}
/*
	if (loaded_paths[root]) {
		throw new Error(
			util.format('attempt to load root %s twice; first execution: %s', root, util.inspect(loaded_paths[root])));
	} else {
		try {
			throw new Error('first execution');
		} catch (err) {
			loaded_paths[root] = err.stack;
		}
	} */
